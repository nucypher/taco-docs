# Table of contents

## Getting Started

* [Introduction to TACo](README.md)
* [How TACo Works](getting-started/key-concepts.md)
* [Quickstart (Testnet)](quickstart-testnet.md)

## For Developers

* [Integrate TACo Into Apps](for-developers/taco-integration/README.md)
  * [Testnets](for-developers/taco-integration/get-started-with-tac.md)
  * [Mainnet Access](for-developers/taco-integration/mainnet-taco-beta-program.md)
  * [Mainnet Deployment](for-developers/taco-integration/deploying-to-mainnet.md)
* [Ecosystem Integrations](for-developers/integrations/README.md)
  * [OrbisDB](for-developers/integrations/orbisdb.md)
  * [Waku](for-developers/integrations/waku.md)
  * [Irys](for-developers/integrations/irys.md)
  * [ComposeDB](for-developers/integrations/composedb.md)
  * [Turbo](for-developers/integrations/turbo.md)
* [API](for-developers/references/README.md)
  * [Encryptor Allowlist](for-developers/references/encryptor-allowlist.md)
  * [Encrypt & Decrypt](for-developers/references/encrypt-and-decrypt-api.md)
  * [Authentication](for-developers/references/authentication/README.md)
    * [Condition Context](for-developers/references/authentication/conditioncontext-and-context-variables.md)
  * [Access Control](for-developers/references/conditions/README.md)
    * [TimeCondition](for-developers/references/conditions/timecondition.md)
    * [RpcCondition](for-developers/references/conditions/rpccondition.md)
    * [ContractCondition](for-developers/references/conditions/contractcondition/README.md)
      * [Use custom contract calls](for-developers/references/conditions/contractcondition/use-custom-contract-calls.md)
      * [Implement access revocation via smart contract](for-developers/references/conditions/contractcondition/implementing-revocation-via-smart-contract.md)
    * [JSON Endpoint Conditions](for-developers/references/conditions/json-endpoint-conditions/README.md)
      * [JsonApiCondition](for-developers/references/conditions/json-endpoint-conditions/jsonapicondition.md)
      * [JsonRpcCondition](for-developers/references/conditions/json-endpoint-conditions/jsonrpccondition.md)
    * [JWT Conditions](for-developers/references/conditions/jwtcondition.md)
    * [Logical Conditions](for-developers/references/conditions/logical-conditions/README.md)
      * [CompoundCondition](for-developers/references/conditions/logical-conditions/condition-set.md)
      * [IfThenElseCondition](for-developers/references/conditions/logical-conditions/ifthenelsecondition.md)
      * [SequentialCondition](for-developers/references/conditions/logical-conditions/sequentialcondition.md)
    * [WIP / Feature Requests](for-developers/references/conditions/wip-feature-requests/README.md)
      * [Any (Major) EVM Chain Condition Support](for-developers/references/conditions/wip-feature-requests/any-major-evm-chain-condition-support.md)
* [Blueprints & Inspiration](for-developers/use-cases/README.md)
  * [Seed phrase recovery & transfer](for-developers/use-cases/seed-phrase-recovery-and-transfer.md)
  * [Digital Rights Management for on-chain assets](for-developers/use-cases/digital-rights-management-for-on-chain-assets.md)
  * [Trustless channels for journalists, archivists & whistleblowers](for-developers/use-cases/trustless-channels-for-journalists-archivists-and-whistleblowers.md)
  * [Crowdsourcing real-world data with trustless contribution](for-developers/use-cases/crowdsourcing-real-world-data-with-trustless-contribution.md)

## For Product Leads

* [Value Propositions](for-product-leads/value-propositions.md)
* [Capabilities & Extensions](for-product-leads/capabilities-and-extensions.md)
* [Use cases](for-product-leads/use-cases/README.md)
  * [Seed phrase recovery & transfer](for-product-leads/use-cases/seed-phrase-recovery-and-transfer.md)
  * [Digital Rights Management for on-chain assets](for-product-leads/use-cases/digital-rights-management-for-on-chain-assets.md)
  * [Trustless channels for journalists, archivists & whistleblowers](for-product-leads/use-cases/trustless-channels-for-journalists-archivists-and-whistleblowers.md)
  * [Crowdsourcing real-world data with trustless contribution](for-product-leads/use-cases/crowdsourcing-real-world-data-with-trustless-contribution.md)
* [Mainnet Fees](for-product-leads/mainnet-fees.md)
* [Trust Assumptions](for-product-leads/trust-assumptions/README.md)
  * [Mainnet Trust Disclosure (Provider Answers)](for-product-leads/trust-assumptions/mainnet-trust-disclosure-provider-answers.md)
  * [Mainnet Trust Model Foundation](for-product-leads/trust-assumptions/mainnet-trust-model-foundation.md)
  * [Trust levers & parameter packages](for-product-leads/trust-assumptions/trust-levers-and-parameter-packages.md)

## Reference

* [Contract Addresses](reference/contract-addresses.md)
* [Architecture](reference/architecture/README.md)
  * [Porter](reference/architecture/porter.md)
* [Github](https://github.com/nucypher/taco-web)
* [TACo Playground](https://playground.taco.build/)
* [TACo Scan](https://tacoscan.com/)

## For Node Operators

* [Getting Set Up](for-node-operators/getting-set-up/README.md)
  * [Minimum System Requirements](for-node-operators/getting-set-up/minimum-system-requirements.md)
  * [Run a TACo Node with Docker](for-node-operators/getting-set-up/run-a-taco-node-with-docker.md)
* [Operations](for-node-operators/operations/README.md)
  * [TACo Node Management](for-node-operators/operations/taco-node-management.md)
  * [TACo Node Recovery](for-node-operators/operations/taco-node-recovery.md)
  * [Stake Authorization](for-node-operators/operations/stake-authorization.md)
* [Duties, Compensation & Penalties](for-node-operators/duties-compensation-and-penalties.md)
* [Run a Porter Instance](for-node-operators/run-a-porter-instance.md)
